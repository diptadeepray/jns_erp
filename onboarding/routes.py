...
from config import CLIENT_MONEY_DISTRIBUTION_CATEGORYWISE
...
def onboarding_home():
    ...
    if request.method == 'POST':
        ...
        if form_type == "add_client":
            ...
            supplier_id = supplier_name[0:3].upper()
            ...
            #         supplier_id = supplier_id[0:7]+str(a)
            #         a+=1
            #     else:
            #         n=1

            c.execute("INSERT INTO all_suppliers (supplier_name, supplier_id) VALUES (?, ?)", (supplier_name, supplier_id))
            conn.commit()
            conn.close()
            return redirect('/onboarding')
            ...
            contractor_id = contractor_name[0:3].upper()

            c.execute("INSERT INTO all_contractors (contractor_name, contractor_id) VALUES (?, ?)", (contractor_name, contractor_id))
            conn.commit()
            conn.close()
            return redirect('/onboarding')
